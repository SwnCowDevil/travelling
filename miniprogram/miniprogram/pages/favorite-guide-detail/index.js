...
Page({
  data:{saved:null,draft:null,editing:false,loading:true,saving:false,error:''},
  async onLoad(query){this.id=Number(query.id);await this.load()},
  async load(){try{const value=await getApp().globalData.api.request({path:`/favorite-guides/${this.id}`});this.setData({saved:value,draft:clone(value),loading:false})}catch(error){this.setData({loading:false,error:error.message||'收藏攻略加载失败'})}},
  ...
  async save(){if(this.data.saving)return;this.setData({saving:true});try{const {draft}=this.data;const value=await getApp().globalData.api.request({method:'PUT',path:`/favorite-guides/${this.id}`,data:{generation_mode:draft.generation_mode,payload:draft.payload}});this.setData({saved:value,draft:clone(value),editing:false});wx.showToast({title:'修改已保存'})}catch(error){wx.showToast({title:error.message||'保存失败，请稍后重试',icon:'none'})}finally{this.setData({saving:false})}},
